...
function setSmartHint(input, suggestionText) {
    const hintEl = document.getElementById('autocompleteHint');
    if (!hintEl || !smartAutocompleteEnabled || !input || input.tagName !== 'TEXTAREA') {
        if (hintEl) hintEl.textContent = '';
        currentSmartHint = null;
        return;
    }
    ...
    if (inputVal && suggestion.toLowerCase().startsWith(inputVal.toLowerCase())) {
        suggestion = suggestion.substring(inputVal.length).trim();
        if (suggestion.length > 0) {
            hintEl.textContent = inputVal + ' ' + suggestion;
        } else {
            hintEl.textContent = '';
        }
    } else {
        hintEl.textContent = suggestionText;
    }
    currentSmartHint = suggestionText;
}

function applySmartHint(input) {
    if (!smartAutocompleteEnabled || !currentSmartHint) return;
    const current = input.value.trim();
    if (!current) return;

    let suggestion = currentSmartHint;
...
function toggleSmartAutocomplete() {
    smartAutocompleteEnabled = !smartAutocompleteEnabled;
    const btn = document.getElementById('btnToggleSmartAutocomplete');
    if (btn) {
        btn.textContent = `Smart Autocompletado: ${smartAutocompleteEnabled ? 'ON' : 'OFF'}`;
        btn.classList.toggle('btn-primary', smartAutocompleteEnabled);
    }
    const active = document.activeElement;
    if (active && active.tagName === 'TEXTAREA') {
        setSmartHint(active, smartAutocompleteEnabled ? currentSmartHint : '');
    } else {
        setSmartHint(null, '');
    }
}
...
function initDiagnosticoAutocomplete() {
    ...
    // Cargar catálogo CIE-10 local al iniciar
    loadCie10Cache();
    ...
    fields.forEach((id) => {
        ...
        const handler = debounce(async (e) => {
            ...
            // Buscar en abreviaturas locales
            ...
            // Buscar en CIE-10 (cache local o API)
            let cieResults = searchCie10Local(query);
            if (!cieResults.length) {
                try {
                    const res = await fetch(`/api/conceptos?q=${encodeURIComponent(query)}`);
                    if (res.ok) cieResults = await res.json();
                } catch (err) {
                    console.error('Error CIE-10 API, se usará solo abreviaturas:', err);
                }
            }

            const combined = [...abrevsMatched, ...cieResults];
            showDiagSuggestions(combined, diagInput);

            if (smartAutocompleteEnabled && combined.length > 0) {
                // Predicción tono bajo: sugerencia del mejor candidato
                const best = combined[0];
                if (best && best.descripcion) {
                    setSmartHint(diagInput, `${best.codigo ? best.codigo + ' ' : ''}${best.descripcion}`);
                } else {
                    setSmartHint(diagInput, '');
                }
            } else {
                setSmartHint(diagInput, '');
            }
    ...
    document.addEventListener('click', (e) => {
        ...
        if (!box) return;
        if (!box.contains(e.target) && !fields.some(id => document.getElementById(id) === e.target)) {
            hideSuggestions();
        }
    });
}

function showSuggestions(results, input, queryTerm) {
    const box = document.getElementById('suggestionsBox');
    ...
    box.innerHTML = limitedResults.map((item, idx) => {
        const safeText = item.replace(/'/g, "\\'").replace(/"/g, '&quot;');
        return `
            <div class="suggestion-item" data-index="${idx}" onclick="applySuggestionFromOption('${safeText}')">
                <span class="sugg-text">${highlightDiagMatch(item, queryTerm)}</span>
            </div>
        `;
    }).join("\n");
...
function applySuggestionFromOption(suggestion) {
    if (!currentAutocompleteTextarea) return;
    applySuggestion(currentAutocompleteTextarea, suggestion);
    currentAutocompleteTextarea.focus();
    hideSuggestions();
}
...
function highlightDiagMatch(text, query) {
...
function handleTextareaAutocompleteKey(e) {
    ...
    if (e.key === 'Tab' && smartAutocompleteEnabled && currentSmartHint) {
        e.preventDefault();
        applySmartHint(targetInput);
        return;
    }
